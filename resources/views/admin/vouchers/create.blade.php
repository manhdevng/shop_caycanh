...
@section('title', 'Tạo mã giảm giá · Cây Cảnh Shop')

@section('content')
<div class="mb-8 flex justify-between items-center">
    <h2 class="text-5xl font-medium gloock text-text-primary tracking-tight">Tạo Mã Giảm Giá</h2>
...
<form action="{{ route('admin.vouchers.store') }}" method="POST">
    ...
    <div class="bg-white rounded-[32px] p-8 border border-green-border shadow-sm">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code') }}" placeholder="VD: SUMMER50" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5] uppercase" required>
            ...
            <div>
                ...
                <select name="discount_type" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" required>
                    ...
                    <option value="fixed" @selected(old('discount_type') === 'fixed')>Số tiền cố định (đ)</option>
                </select>
                @error('discount_type')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0.01" name="discount_value" value="{{ old('discount_value') }}" placeholder="VD: 10 (nếu %) hoặc 30000 (nếu đ)" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" required>
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex flex-wrap gap-4 mb-3">
                    <label class="inline-flex items-center gap-2 text-sm text-text-primary">
                        <input type="radio" name="scope_type" value="all" class="scope-type-radio w-4 h-4" {{ old('scope_type', 'all') === 'all' ? 'checked' : '' }}>
                        Toàn bộ sản phẩm
                    </label>
                    <label class="inline-flex items-center gap-2 text-sm text-text-primary">
                        <input type="radio" name="scope_type" value="products" class="scope-type-radio w-4 h-4" {{ old('scope_type') === 'products' ? 'checked' : '' }}>
                        Chỉ một số sản phẩm
                    </label>
                    <label class="inline-flex items-center gap-2 text-sm text-text-primary">
                        <input type="radio" name="scope_type" value="categories" class="scope-type-radio w-4 h-4" {{ old('scope_type') === 'categories' ? 'checked' : '' }}>
                        Chỉ một số danh mục
                    </label>
                </div>
                @error('scope_type')
                ...
                <div id="scope-products-box" style="display:none" class="mb-3">
                    ...
                    <select name="product_ids[]" multiple size="6" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                        @foreach(($allProducts ?? collect()) as $p)
                            <option value="{{ $p->id }}" @selected(in_array($p->id, old('product_ids', [])))>{{ $p->name }}</option>
                        @endforeach
                    </select>
                ...
                <div id="scope-categories-box" style="display:none">
                    ...
                    <select name="category_ids[]" multiple size="6" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                        @foreach(($allCategories ?? collect()) as $c)
                            <option value="{{ $c->id }}" @selected(in_array($c->id, old('category_ids', [])))>{{ $c->name }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="number" step="1" min="0" name="min_order_amount" value="{{ old('min_order_amount') }}" placeholder="Để trống nếu không yêu cầu" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
            ...
            <div>
                ...
                <input type="number" step="1" min="1" name="usage_limit" value="{{ old('usage_limit') }}" placeholder="Để trống = không giới hạn" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                @error('usage_limit')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                @error('starts_at')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                @error('expires_at')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="flex justify-end gap-4 mt-6">
        ...
        <button type="submit" class="px-10 py-3 bg-green-primary text-white rounded-pill font-medium hover:bg-green-accent transition-all flex items-center gap-2">
            <i data-lucide="save" class="w-5 h-5"></i> Lưu mã giảm giá
        </button>
    </div>
</form>
