...
@section('title', 'Thêm câu hỏi thường gặp · Cây Cảnh Shop')

@section('content')
<div class="mb-8 flex justify-between items-center">
    <h2 class="text-5xl font-medium gloock text-text-primary tracking-tight">Thêm FAQ</h2>
...
<form action="{{ route('admin.faqs.store') }}" method="POST">
    ...
    <div class="bg-white rounded-[32px] p-8 border border-green-border shadow-sm">
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input type="text" name="question" value="{{ old('question') }}" maxlength="255" placeholder="VD: Cây có dễ chăm sóc không?" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" required>
                @error('question')
            ...
            <div>
                ...
                <textarea name="answer" rows="6" placeholder="Nhập nội dung trả lời..." class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">{{ old('answer') }}</textarea>
                @error('answer')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" min="0" value="{{ old('sort_order', 0) }}" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
    ...
    <div class="flex justify-end gap-4 mt-6">
        ...
        <button type="submit" class="px-10 py-3 bg-green-primary text-white rounded-pill font-medium hover:bg-green-accent transition-all flex items-center gap-2">
            <i data-lucide="save" class="w-5 h-5"></i> Lưu FAQ
        </button>
    </div>
</form>
@endsection
