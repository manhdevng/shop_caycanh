...
@section('title', 'Thêm sản phẩm · Cây Cảnh Shop')

@php
    $oldVariants = old('variants', []);
    $oldProductType = old('product_type', 'plant');
    $oldPricingMode = old('pricing_mode', !empty($oldVariants) ? 'variants' : 'single');
@endphp

@section('content')
<div class="mb-8 flex justify-between items-center">
    <h2 class="text-5xl font-medium gloock text-text-primary tracking-tight">Thêm Sản Phẩm Mới</h2>
...
<form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="bg-white rounded-[32px] p-8 border border-green-border shadow-sm mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                ...
                <input type="text" name="name" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" value="{{ old('name') }}" required>
            </div>
            ...
            <div>
                ...
                <input type="number" name="weight" min="1" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" value="{{ old('weight', 200) }}" required>
            ...
            <div>
                ...
                <input type="number" name="stock" min="0" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" value="{{ old('stock', 0) }}" required>
                @error('stock')
            ...
            <div>
                ...
                <div class="mt-4 hidden" id="mainImagePreviewContainer">
                    <img id="mainImagePreview" class="w-40 h-40 object-cover rounded-2xl shadow-sm border border-green-border/50">
                </div>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-text-primary mb-2 mono">Mô tả sản phẩm</label>
                <textarea name="description" rows="4" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">{{ old('description') }}</textarea>
            </div>
        </div>
    </div>
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto">
                        @forelse($group->children as $childCategory)
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input type="checkbox" name="categories[]" value="{{ $childCategory->id }}" class="w-4 h-4 rounded border-green-border/50 focus:ring-green-primary" {{ (is_array(old('categories')) && in_array($childCategory->id, old('categories'))) ? 'checked' : '' }}>
    ...
    <div class="bg-white rounded-[32px] p-8 border border-green-border shadow-sm mb-6">
        ...
        <div id="singlePriceBlock">
            ...
            <input type="number" name="base_price" min="0" class="w-full md:w-64 rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]" value="{{ old('base_price') }}">
        ...
        <div id="variantsBlock">
            <div class="mb-4">
                ...
                <input type="text" name="variant_label" list="variantLabelOptions" value="{{ old('variant_label') }}" placeholder="Để trống dùng mặc định theo loại sản phẩm" class="w-full md:w-96 rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
            </div>
            ...
            <div class="overflow-x-auto border border-green-border/50 rounded-2xl bg-white shadow-sm overflow-hidden">
                <table class="w-full text-left border-collapse" id="variantsTable">
                    ...
                    <tbody id="variantsContainer" class="divide-y divide-green-border/20">
                        ...
                        <tr class="group variant-row hover:bg-green-background/30 transition-colors" id="variant-row-{{ $index }}">
                            ...
                            <td class="py-3 px-4">
                                <input type="number" name="variants[{{ $index }}][price]" min="1000" value="{{ $row['price'] ?? '' }}" placeholder="vd: 150000" class="w-full rounded-xl border-green-border/50 border px-3 py-2 bg-white focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none text-sm text-text-primary" required>
                            </td>
                            <td class="py-3 px-4">
                ...
                <div id="noVariantsMsg" class="text-center py-8 text-text-secondary italic mono text-sm {{ !empty($oldVariants) ? 'hidden' : '' }}">Chưa có phân loại nào được thêm.</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[32px] p-8 border border-green-border shadow-sm mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" id="is_active" class="w-5 h-5 rounded border-green-border/50 focus:ring-green-primary" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <select name="badge" class="w-full rounded-xl border-green-border/50 border px-4 py-3 focus:ring-2 focus:ring-green-primary focus:border-green-primary outline-none transition-all text-text-primary bg-[#f8f9f5]">
                    <option value="" {{ old('badge') === null || old('badge') === '' ? 'selected' : '' }}>Tự động</option>
                    @foreach(\App\Models\Product::BADGES as $badgeValue => $badgeLabel)
                        <option value="{{ $badgeValue }}" {{ old('badge') === $badgeValue ? 'selected' : '' }}>{{ $badgeLabel }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="flex justify-end gap-4 mb-12">
        ...
        <button type="submit" class="px-10 py-3 bg-green-primary text-white rounded-pill font-medium hover:bg-green-accent transition-all flex items-center gap-2">
            <i data-lucide="save" class="w-5 h-5"></i> Lưu sản phẩm
        </button>
    </div>
</form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let variantIndex = {{ !empty($oldVariants) ? max(array_keys($oldVariants)) + 1 : 0 }};
